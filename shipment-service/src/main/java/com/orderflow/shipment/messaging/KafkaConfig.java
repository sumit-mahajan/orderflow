...
/** Kafka wiring for shipment-service — mirrors payment-service pattern exactly. */
@Configuration
public class KafkaConfig {
  ...
  @Value("${spring.kafka.consumer.group-id:shipment-service}")
  ...
  @Bean
  public NewTopic commandsShipmentTopic() {
    return TopicBuilder.name(Topics.COMMANDS_SHIPMENT).partitions(3).replicas(1).build();
  }

  @Bean
  public NewTopic eventsShipmentTopic() {
    return TopicBuilder.name(Topics.EVENTS_SHIPMENT).partitions(3).replicas(1).build();
  }
}
