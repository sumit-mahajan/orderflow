...
/**
 * Kafka wiring kept explicit (rather than pure application.yml magic) so the JSON contract is
 * obvious: producer sends JSON without type headers; the command consumer always deserializes to the
 * single {@link CommandMessage} type and switches on its {@code type} field.
 */
@Configuration
